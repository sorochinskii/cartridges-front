...
export function useFileUpload(fetchCallback) {
    ...
    const uploadingVendors = ref(false)
    ...
    const uploadVendorsFromCsv = async () => {
        uploadingVendors.value = true
        try {
            await api.post('/vendors/batch/create', uploadReadyData.value)
